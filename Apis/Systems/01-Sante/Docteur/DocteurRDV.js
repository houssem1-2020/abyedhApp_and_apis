...
const docteurRDVRouter = express.Router()
...
/*####################################[LOGIN & LANDING ]###########################*/
	...
	/*log in*/
	docteurRDVRouter.post('/LogIn', (req, res) => {
		  const logInD = req.body.LoginData;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
		  let sql = `SELECT * FROM 01_docteur_caisses WHERE Identifiant = '${logInD.Log}' AND Password  = '${logInD.Pwd}'` ;
		  connection.query(sql, (err, rows, fields) => {
		    if (err) throw err
		    res.json(rows[0])
	...
	docteurRDVRouter.post('/main/position', (req, res) => {
				...
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `INSERT INTO 01_docteur_caisses_position(Camion_ID,jour,heur,Position) VALUES ('${camId}','${Today}','${ToTime}','${position}') ON DUPLICATE KEY UPDATE jour = '${Today}',  heur = '${ToTime}' , Position = '${position}' ;`;
				connection.query(sql, (err, rows, fields) => {
					if (err) throw err
					res.json(rows);
				}); 
...
/*####################################[CAISSE SIMPLE ]#############################*/
	...
	docteurRDVRouter.post('/nv/stock', (req, res) => {
		  const PID = req.body.forPID;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
		  let sql = `SELECT * FROM 01_docteur_menu WHERE PID = '${PID}' ` ;
		  connection.query(sql, (err, rows, fields) => {
		    if (err) throw err
		    res.json(rows);
	...
	docteurRDVRouter.post('/nv/ajouter', (req, res) => {
		(async() => {
		  	    let PID = req.body.forPID;
				let factureD = req.body.factureD;
				let FID = await GenerateID(1111111111,`01_docteur_factures`,'T_ID');
				let articleL = JSON.stringify(factureD.articles)
				let Today = new Date().toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `INSERT INTO 01_docteur_factures (PID,T_ID,Cre_Date,Camion,C_Name,Tota, Chauffeur,Articles) 
	...
	docteurRDVRouter.post('/nv/ajouter', (req, res) => {
		(async() => {
				...
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `INSERT INTO 01_docteur_factures (PID, T_ID,Caisse_ID,Final_Value,Espece, T_Date, T_Time,Client,State, Paye_Bons, Articles) 
	...
	/* modifier un facture */
	docteurRDVRouter.post('/nv/payee/espece', (req, res) => {
		  let PID = req.body.forPID;
		  let espece = req.body.espece
		  let FID = req.body.fid;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
		  let sql = `UPDATE 01_docteur_factures
	...
	/* modifier un facture */
	docteurRDVRouter.post('/nv/payee/bons', (req, res) => {
		  let PID = req.body.forPID;
	...
	/* modifier un facture */
	docteurRDVRouter.post('/nv/client/credit', (req, res) => {
		  let PID = req.body.forPID;
	...
	/* modifier un facture */
	docteurRDVRouter.post('/nv/client/fidelite', (req, res) => {
		  let PID = req.body.forPID;
		  let clientId = req.body.clientId
		  let FID = req.body.fid;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
	...
	/* modifier un facture */
	docteurRDVRouter.post('/nv/openCaisse', (req, res) => {
		  let PID = req.body.forPID;
...
/*####################################[MES FACT]###################################*/
	...
	/* selctioner mes 01_docteur_factures [in between] */
	docteurRDVRouter.post('/mf', (req, res) => {
		  const TAG = req.body.tag;
		  const camId = req.body.camId;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
		  let sql = `SELECT * 
		  			 FROM 01_docteur_factures 
		  			 LEFT JOIN 01_docteur_clients ON 01_docteur_factures.C_Name = 01_docteur_clients.CL_ID
		  			 WHERE 01_docteur_factures.Camion = ${camId} ORDER BY 01_docteur_factures.Cre_Date DESC LIMIT 50` ;
		  connection.query(sql, (err, rows, fields) => {
		    if (err) throw err
		    res.json(rows);
	...
	/* selectioner une 01_docteur_factures */
	docteurRDVRouter.post('/mf/select', (req, res) => {
		  const TAG = req.body.tag;
		  const camId = req.body.camId;
		  ...
		  let sql = `SELECT *
		  			 FROM 01_docteur_factures 
		  			 LEFT JOIN 01_docteur_clients ON 01_docteur_factures.C_Name = 01_docteur_clients.CL_ID
		  			 WHERE 01_docteur_factures.Camion =  ${camId}  AND 01_docteur_factures.F_ID = ${FID} ` ;
		  connection.query(sql, (err, rows, fields) => {
		    if (err) throw err
		    res.json(rows);
	...
	/* modifier un facture */
	docteurRDVRouter.post('/mf/modifier', (req, res) => {
		  let PID = req.body.forPID;
...
/*####################################[STOCK]######################################*/
	/*Suivie un article */
	docteurRDVRouter.post('/sk/suivie', (req, res) => {
		    let PID = req.body.forPID;
	...
	/*Selectionner Fonds */
	docteurRDVRouter.post('/sk/fonds', (req, res) => {
		  const TAG = req.body.tag;
		  const camId = req.body.camId;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
	...
	docteurRDVRouter.post('/sk/fonds/info', (req, res) => {
		  const TAG = req.body.tag;
		  const camId = req.body.camId;
		  const FID = req.body.fid;
		  connection.changeUser({database : 'dszrccqg_system'}, () => {});
	...
	/*Regler Ventes*/
	docteurRDVRouter.post('/sk/reglemment', (req, res) => {
		  let PID = req.body.forPID  
	...
	docteurRDVRouter.post('/sk/reglemment/check', (req, res) => {
		  let PID = req.body.forPID;
		  let camId = req.body.camId;
	...
	/*Regler Ventes*/
	docteurRDVRouter.post('/sk/reglemment/enregistrer', (req, res) => {
		  	    let PID = req.body.forPID;
				let Camion = req.body.camId;
				let today = req.body.jour;
				//let today =  new Date().toISOString().split('T')[0]
...
/*####################################[VENTES]#####################################*/
	...
	docteurRDVRouter.post('/mv/resumer', (req, res) => {
				let PID = req.body.forPID
				let camId = req.body.camId
				let Today =  new Date().toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )
				function FetchStock() {
	...
	/* selctioner mes vente auhoudhui */
	docteurRDVRouter.post('/mv/resumer/jour', (req, res) => {
				let PID = req.body.forPID
				let camId = req.body.camId
	...
	/* selctioner mes vente auhoudhui */
	docteurRDVRouter.post('/mv/article', (req, res) => {
		   let PID = req.body.forPID
	       let camId = req.body.camId
...
/*####################################[CLIENT]#####################################*/
	...
	docteurRDVRouter.post('/client/reglemment', async function(req, res, next) {
	  	    let PID = req.body.forPID;
			let clientId = req.body.clientId;
...
/*####################################[RECETTE]####################################*/
	/* Ajouter Depense */
	docteurRDVRouter.post('/rt/depense/list', (req, res) => {   
		  	    let PID = req.body.forPID;
				let Today = new Date().toISOString().split('T')[0]
				let camId = req.body.camId;
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `SELECT * FROM 01_docteur_caisses_depenses WHERE Caisse_ID = ${camId} AND D_Date = '${Today}'`;
				connection.query(sql, (err, rows, fields) => {
				if (err) throw err
				res.json(rows);
	...
	/* Ajouter Depense */
	docteurRDVRouter.post('/rt/depense/ajouter', (req, res) => {
		  	    let PID = req.body.forPID;
				let depD = req.body.depenseD;
				let Today = new Date().toISOString().split('T')[0]
				let camId = req.body.camId;
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `INSERT INTO 01_docteur_caisses_depenses (Caisse_ID,Depense,Valeur, Description, D_Date) 
	...
	/* Supprimer Depense */
	docteurRDVRouter.post('/rt/depense/supprimer', (req, res) => {
		  	    let PID = req.body.forPID;
				let ID = req.body.depId;
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `DELETE FROM 01_docteur_caisses_depenses WHERE PK = ${ID}`;
				connection.query(sql, (err, rows, fields) => {
				if (err) throw err
				res.json({ID:ID});
	...
	/*Imprimer Recette */
	docteurRDVRouter.post('/rt/imprimer', (req, res) => {
		    let PID = req.body.forPID;
		    let camionID = req.body.camId;
		    let Today = new Date().toISOString().split('T')[0]
		    ...
		    function CalculateRecette() {
			      return new Promise((resolve, reject) => {
			      	...
			      	let sql = `SELECT SUM(Final_Value) AS RCT
			                  FROM  01_docteur_factures WHERE Caisse_ID = ${camionID} AND T_Date = '${Today}' AND State = 'Payee' `;
				       connection.query(sql, (err, rows, fields) => {
				          if (err) return reject(err);
		    ...
		    function CalculateDepense() {
			      return new Promise((resolve, reject) => {
			      	...
			      	let sql = `
			                   ...
			                   FROM 01_docteur_caisses_depenses WHERE Caisse_ID = ${camionID} AND D_Date = '${Today}'`;
				       connection.query(sql, (err, rows, fields) => {
				          if (err) return reject(err);
		    ...
		    function CalculateBons() {
			      return new Promise((resolve, reject) => {
				      	connection.changeUser({database : 'dszrccqg_system'}, () => {});
				      	let sql = `SELECT Paye_Bons  FROM 01_docteur_factures WHERE Caisse_ID = ${camionID} AND T_Date = '${Today}' AND Paye_Bons != ''`; 
				        connection.query(sql, (err, rows, fields) => {
					        if (err){ throw err}
	...
	/*Imprimer Recette */
	docteurRDVRouter.post('/rt/imprimer/temp', (req, res) => {
		    let PID = req.body.forPID;
		    let camionID = req.body.camId;
		    let Today = new Date().toISOString().split('T')[0]
		    ...
		    function CalculateDepense() {
			      return new Promise((resolve, reject) => {
			      	connection.changeUser({database : 'dszrccqg_system'}, () => {});
			      	let sql = `
			      			   SELECT * FROM 01_docteur_caisses_depenses WHERE Caisse_ID = ${camionID} AND D_Date = '${Today}'`;
				       connection.query(sql, (err, rows, fields) => {
				          if (err) return reject(err);
				          resolve(rows);
	...
	/* Ajouter Depense */
	docteurRDVRouter.post('/rt/factures', (req, res) => {   
		  	    let PID = req.body.forPID;
				let Today = new Date().toISOString().split('T')[0]
				let caisseId = req.body.caisseId;
				connection.changeUser({database : 'dszrccqg_system'}, () => {});
				let sql = `SELECT * , COALESCE(01_docteur_clients.CL_Name, 'PASSAGER') AS CL_Name , 01_docteur_factures.State AS Pay_State FROM 01_docteur_factures 
		                     LEFT JOIN 01_docteur_clients ON 01_docteur_factures.Client = 01_docteur_clients.CL_ID 
		                     LEFT JOIN 01_docteur_caisses ON 01_docteur_factures.Caisse_ID = 01_docteur_caisses.C_ID 
	...
	/* Ajouter Depense */
	docteurRDVRouter.post('/rt/factures/select', (req, res) => {   
		  	    let PID = req.body.forPID;
				let Today = new Date().toISOString().split('T')[0]
				let FID = req.body.FID;
				let caisseId = req.body.caisseId;
				...
				let sql = `SELECT * , COALESCE(01_docteur_clients.CL_Name, 'PASSAGER') AS CL_Name ,01_docteur_factures.State AS Pay_State FROM 01_docteur_factures 
		                     LEFT JOIN 01_docteur_clients ON 01_docteur_factures.Client = 01_docteur_clients.CL_ID 
		                     LEFT JOIN 01_docteur_caisses ON 01_docteur_factures.Caisse_ID = 01_docteur_caisses.C_ID 
		                     WHERE 01_docteur_factures.Caisse_ID = ${caisseId} AND 01_docteur_factures.T_ID = '${FID}'`;
				connection.query(sql, (err, rows, fields) => {
				if (err) throw err
				res.json(rows);
...
/*####################################[COMMANDES]##################################*/
	/* selectionner tous les 01_docteur_factures */
	docteurRDVRouter.post('/commande/search', (req, res) => {
	        const PID = req.body.forPID;
		    const FID = req.body.FID;
		    let Today = new Date().toISOString().split('T')[0]
	        connection.changeUser({database : 'dszrccqg_system'}, () => {});
	        let sql = `SELECT *  FROM 01_docteur_factures  WHERE T_ID = '${FID}' AND Is_Commande != '' `;
	        connection.query(sql, (err, rows, fields) => {
	        if (err){ throw err}
	        res.json(rows[0]);
	      })
	...
	/* selectionner tous les 01_docteur_factures */
	docteurRDVRouter.post('/commande/verifier', (req, res) => {
	        const PID = req.body.forPID;
		    let Today = new Date().toISOString().split('T')[0]
	        ...
	        let sql = `SELECT * FROM dszrccqg_communications.01_docteur_commande 
                       ...
                       WHERE  dszrccqg_communications.01_docteur_commande.PID = '${PID}' AND ( dszrccqg_communications.01_docteur_commande.State = 'W' OR  dszrccqg_communications.01_docteur_commande.State = 'S') ` ;
	        connection.query(sql, (err, rows, fields) => {
	        if (err){ throw err}
	        res.json(rows);
	      })
	...
	/* selectionner tous les 01_docteur_factures */
	docteurRDVRouter.post('/commande/select', (req, res) => {
	        const PID = req.body.forPID;
	        ...
	        let sql = `SELECT * FROM dszrccqg_communications.01_docteur_commande 
                       INNER JOIN dszrccqg_profile.user_general_data ON dszrccqg_communications.01_docteur_commande.UID = dszrccqg_profile.user_general_data.UID 
                       LEFT JOIN dszrccqg_system.01_docteur_clients ON dszrccqg_communications.01_docteur_commande.UID = dszrccqg_system.01_docteur_clients.Releted_UID 
                       WHERE  dszrccqg_communications.01_docteur_commande.PID = '${PID}' AND dszrccqg_communications.01_docteur_commande.R_ID = '${RID}'` ;
	        connection.query(sql, (err, rows, fields) => {
	        if (err){ throw err}
	        res.json(rows);
	      })
...
/*&&&&&&&&&&&&&&&&&[Mettre A Jour]&&&&&&&&&&&&&&*/
  /* fetch main Tools */
	docteurRDVRouter.post('/update', (req, res) => {
	        let PID = req.body.forPID
	        let Today = new Date().toISOString().split('T')[0]
	        function FetchStock() {
			    return new Promise((resolve, reject) => {
		    ...
		    function FetchFacture() {
			    ...
			    return new Promise((resolve, reject) => {
			      	connection.changeUser({database : 'dszrccqg_system'}, () => {});
			      	let sql = `SELECT * FROM 01_docteur_factures WHERE PID = '${PID}'`;
			       connection.query(sql, (err, rows, fields) => {
				       if (err) return reject(err);
				       resolve(rows);
		      		})
		    ...
		    function FetchClient() {
			    ...
			    return new Promise((resolve, reject) => {
			      	connection.changeUser({database : 'dszrccqg_system'}, () => {});
			      	let sql = `SELECT * FROM 01_docteur_clients WHERE PID = '${PID}'`;
			       connection.query(sql, (err, rows, fields) => {
				       if (err) return reject(err);
				       resolve(rows);
		      		})
...
module.exports = docteurRDVRouter